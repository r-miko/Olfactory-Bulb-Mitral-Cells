# Performs analysis on the MC output results
...
for i in range(1, 5):
	...
	for l,j in enumerate(strengths):
		...
		for m,k in enumerate(frequencies):
			c = str(k) + "/"
			latency = np.load(a + b + c + "MC_First_spike_latency.npy")
			spiketimes = np.load(a + b + c + "MC_Spiketimes.npy")
			intraburst_freq = np.load(a + b + c + "MC_Interspike_frequencies.npy")
...
# SAVE
np.save("feature_data_MC.npy",data)
