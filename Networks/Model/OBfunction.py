...
# MODEL
# Model parameters
def runOB(strength,frequency,mc_pgc_excitation, pgc_mc_inhibition,pgc_stim,record_vars,syn_vars,directory,filename):
	celsius = 35
	AMPAgmax = 10.0*2e-3
	AMPAalpha = 1.0
	AMPAbeta = 1.0/5.5
	...
	pgc_input_current = (np.ones((tstop,))*[np.cos((t/1000.0)*(2*np.pi*frequency)) for t in time]*c2 + strength)*factor
